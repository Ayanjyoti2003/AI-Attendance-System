...
const PG_HEALTH_CHECK_TIMEOUT = 600000; // 10 minutes (600 seconds)
...
/**
 * Known DLL names that are part of the Microsoft Visual C++ Redistributable.
 * If any of these appear in error messages or stderr, the VC++ runtime is missing.
 */
...
/**
 * Windows exit code for STATUS_DLL_NOT_FOUND.
 * When a process exits with this code, it means a required DLL was not found.
 * Node.js reports this as signed: -1073741515 (0xC0000135 interpreted as int32).
 */
const STATUS_DLL_NOT_FOUND_SIGNED = -1073741515;
const STATUS_DLL_NOT_FOUND_UNSIGNED = 0xC0000135;

/**
 * Analyze an error message, exit code, and captured logs to determine if the
 * failure is consistent with a missing Microsoft Visual C++ Runtime.
 *
 * @param {Object} opts
 * @param {string} [opts.errorMessage] - Error message text
 * @param {number|null} [opts.exitCode] - Process exit code
 * @param {string} [opts.logs] - Captured startup logs
 * @returns {{ detected: boolean, detail: string }}
 */
function detectVCRuntimeMissing({ errorMessage = '', exitCode = null, logs = '' } = {}) {
  const combined = `${errorMessage}\n${logs}`.toLowerCase();

  // Check for STATUS_DLL_NOT_FOUND exit code
  if (exitCode === STATUS_DLL_NOT_FOUND_SIGNED ||
      exitCode === STATUS_DLL_NOT_FOUND_UNSIGNED ||
      (exitCode !== null && (exitCode >>> 0) === STATUS_DLL_NOT_FOUND_UNSIGNED)) {
    return {
      detected: true,
      detail: `Process exited with STATUS_DLL_NOT_FOUND (exit code ${exitCode} / 0x${(exitCode >>> 0).toString(16).toUpperCase()}).`,
    };
  }

  // Check for known VC++ runtime DLL names in error output
  for (const dll of VCRUNTIME_DLLS) {
    if (combined.includes(dll.toLowerCase())) {
      return {
        detected: true,
        detail: `Error output references missing runtime DLL: ${dll}`,
      };
    }
  }

  // Check for generic DLL-not-found patterns
  if (combined.includes('status_dll_not_found') ||
      combined.includes('0xc0000135') ||
      combined.includes('the code execution cannot proceed because') ||
      combined.includes('dll was not found')) {
    return {
      detected: true,
      detail: 'Error output contains DLL-not-found indicators.',
    };
  }
...
/**
 * Build a descriptive VC++ runtime missing message for the user.
 */
function buildVCRuntimeMissingMessage(detail) {
  return (
    `\n════════════════════════════════════════════════════════════════\n` +
...
function resolvePaths(appRef) {
  ...
  if (isPackaged) {
    ...
    if (process.platform === "win32") {
      const norm = path.resolve(dataDir).toLowerCase();
...
function readAppConfig(appRef) {
  ...
  if (process.platform === "win32" && isPackaged) {
    ...
    if (process.platform === "win32") {
      const norm = path.resolve(configDir).toLowerCase();
      if (norm.includes("program files") || norm.includes("system32")) {
        const base = process.env.PROGRAMDATA || "C:\\ProgramData";
        configDir = path.join(base, "AI Attendance System", "config");
      }
...
function saveCredentialsViaCli(appRef, password) {
  ...
  log("=== config_cli execution diagnostics ===");
  ...
  log(`AI_ATTENDANCE_PACKAGED: "${env.AI_ATTENDANCE_PACKAGED}"`);
  log(`PYTHONNOUSERSITE: "${env.PYTHONNOUSERSITE}"`);
...
function startPostgres(appRef) {
  ...
  if (shouldSpawn) {
    ...
    postgresProcess.on("error", (err) => {
      ...
      // Check if this is a VC++ runtime issue
      const vcCheck = detectVCRuntimeMissing({ errorMessage: err.message });
      if (vcCheck.detected) {
        const vcMsg = buildVCRuntimeMissingMessage(vcCheck.detail);
        log(vcMsg);
        postgresStartupLogs.push(vcMsg);
      }
    });
  ...
  return new Promise((resolve, reject) => {
    ...
    const check = () => {
      ...
      if (Date.now() - lastWaitLogTime > 5000) {
        log(`Waiting for PostgreSQL to accept connections... (${elapsedSec}s elapsed)`);
      ...
      if (hasExited || (isStarted === false && postgresProcess === null && !postgresStartupLogs.getLogs().includes("healthy"))) {
        ...
        // Parse exit code from exitDetails for VC++ check
        let parsedExitCode = null;
        const exitCodeMatch = (details || '').match(/exit code ([-\d]+)/);
        if (exitCodeMatch) {
          parsedExitCode = parseInt(exitCodeMatch[1], 10);
        }

        // Check if this failure is due to missing VC++ runtime
        const vcCheck = detectVCRuntimeMissing({
        ...
        if (vcCheck.detected) {
          diagnosticMsg += buildVCRuntimeMissingMessage(vcCheck.detail);
        }

        reject(new Error(diagnosticMsg));
        return;
      }
      ...
      if (Date.now() - startTime > PG_HEALTH_CHECK_TIMEOUT) {
        ...
        // Check if the timeout could be due to missing VC++ runtime
        const vcCheck = detectVCRuntimeMissing({
          exitCode: result.status,
          logs: logs || '',
          errorMessage: lastCheck,
        });
        ...
        if (vcCheck.detected) {
          timeoutMsg += buildVCRuntimeMissingMessage(vcCheck.detail);
        }
...
/**
 * Compile detailed PostgreSQL diagnostics for troubleshooting backend startup failures.
 */
function getPostgresDiagnostics(appRef) {
  const { pgBinDir, dataDir } = resolvePaths(appRef);
  const config = readAppConfig(appRef);
  const port = (config && config.database && config.database.port) ? config.database.port : PG_DEFAULT_PORT;
  
  let processStatus = "Not spawned by manager";
  if (postgresProcess) {
    const pid = postgresProcess.pid;
    const running = isPidRunning(pid);
    processStatus = `Spawned (PID: ${pid}, Active: ${running})`;
  } else if (isStarted) {
    processStatus = "Marked as started/reused";
  }

  let portOccupied = "Unknown";
  try {
    const occupancy = checkPortOccupancy(port);
    portOccupied = occupancy.occupied ? `Yes (PID: ${occupancy.pid}, Name: ${occupancy.processName})` : "No";
  } catch (e) {
    portOccupied = `Error checking: ${e.message}`;
  }

  let pgIsReadyStatus = "Unknown";
  try {
    const result = spawnPostgresSync(pgBinDir, "pg_isready", [
      "-h", "127.0.0.1",
      "-p", String(port),
    ...
    pgIsReadyStatus = `Exit code ${result.status}. Stdout: ${(result.stdout || "").trim()}. Stderr: ${(result.stderr || "").trim()}`;
  } catch (e) {
    pgIsReadyStatus = `Error running: ${e.message}`;
  }

  const startupLogs = postgresStartupLogs.getLogs() || "No startup logs captured.";

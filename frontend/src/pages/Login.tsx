...
export default function Login() {
    ...
    async function handleLogin(e: React.FormEvent) {
        ...
        try {
            ...
            localStorage.setItem("must_change_password", response.data.must_change_password ? "true" : "false");
    ...
    async function handleRecoverAdmin(e: React.FormEvent) {
        ...
        try {
            await api.post("/api/recover-admin", {
                recovery_key: recoveryKey.trim(),
                new_password: newAdminPassword
            });
            setRecoverySuccess("Recovery successful. Administrator password reset. Please log in with your new password.");
            setError("");
        } catch (err: any) {
            if (err.response?.data?.detail) {
                setError(err.response.data.detail);
            } else {
                setError("Recovery failed. Check your Recovery Key.");
            }
        } finally {
            setLoading(false);
        }
    ...
    return (
        <div className="min-h-full flex items-center justify-center bg-zinc-950 relative overflow-hidden select-none">
            ...
            <div className="relative w-full max-w-md mx-4 animate-slide-up">
                <div className="bg-zinc-900 border border-zinc-800 rounded-3xl shadow-2xl shadow-black/50 p-8 md:p-10">
                    ...
                    <div className="flex flex-col items-center mb-8">
                        ...
                        <h1 className="text-2xl font-black text-white tracking-tight">
                            {isRecoveryMode ? "Recover Administrator" : "AI Attendance"}
                        </h1>
                        <p className="text-zinc-550 text-sm mt-1.5 text-center">
                            {isRecoveryMode 
                                ? "Use your offline Recovery Key to reset the SUPER_ADMIN password" 
                    ...
                    {recoverySuccess ? (
                        ...
                        /* Recovery Form */
                        <form onSubmit={handleRecoverAdmin} className="flex flex-col gap-5 animate-fade-in">
                            {/* Recovery Key */}
                            <div className="flex flex-col gap-2">
                                <label className="text-xs font-bold uppercase tracking-wider text-zinc-550 dark:text-zinc-400">
                                    Recovery Key
                                </label>
                                <input
                                    type="text"
                                    ...
                                    className="w-full px-4 py-3 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-200 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 placeholder-zinc-650 transition-all font-mono"
                                    required
                                    autoFocus
                                />
                            </div>
                            ...
                            {/* New Password */}
                            <div className="flex flex-col gap-2">
                                <label className="text-xs font-bold uppercase tracking-wider text-zinc-555 dark:text-zinc-400">
                                    New Admin Password
                                </label>
                                <input
                                    type="password"
                        ...
                        <form onSubmit={handleLogin} className="flex flex-col gap-5">
                            {/* Username */}
                            <div className="flex flex-col gap-2">
                                <label htmlFor="login-username" className="text-xs font-bold uppercase tracking-wider text-zinc-555 dark:text-zinc-400">
                                    Username
                                </label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-4 py-3 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-200 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 placeholder-zinc-650 transition-all"
                                        required
                                        autoFocus
                                    />
                                </div>
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex justify-between items-center">
                                    <label htmlFor="login-password" className="text-xs font-bold uppercase tracking-wider text-zinc-555 dark:text-zinc-400">
                                        Password
                                    </label>
                                    <button
                                        type="button"
                                        ...
                                        className="text-xs font-bold text-blue-500 hover:text-blue-400 hover:underline cursor-pointer bg-transparent border-0"
                                    >
                                        Forgot Password?
                                    </button>
                                </div>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-10 pr-12 py-3 rounded-xl bg-zinc-950 border border-zinc-800 text-zinc-200 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500/30 placeholder-zinc-655 transition-all"
                                        required
                                    />
                                    <button
                                        type="button"
                ...
                {/* Footer */}
                <p className="text-center text-zinc-650 text-xs mt-6 font-medium">
